...
from extras.momentum import PerceptronMomentum
from extras.crossval import grid_search, train_final_model
...
print("\n=== доп. задание 4  momentum sgd ===")
beta_values = [0.5, 0.9, 0.99]
momentum_models = {}

for beta in beta_values:
    m = PerceptronMomentum(X_train.shape[1])
    m.fit(X_train, y_train, X_test, y_test,
          epochs=100, lr=0.1, batch_size=32, beta=beta)
    momentum_models[f'β={beta}'] = m
    print(f"β={beta}: train={m.accuracy(X_train, y_train):.4f}  test={m.accuracy(X_test, y_test):.4f}")

# обычный sgd для сравнения
m_sgd = Perceptron(X_train.shape[1])
m_sgd.fit(X_train, y_train, X_test, y_test, epochs=100, lr=0.1, batch_size=32)
momentum_models['sgd (β=0)'] = m_sgd

plot_loss_curves(momentum_models, title='momentum vs sgd')

print("\n=== доп. задание 5  кросс-валидация ===")

# загружаем ненормализованные данные для кросс-валидации
X_raw, y_raw = load_data()

lr_grid    = [0.001, 0.01, 0.1, 0.5]
batch_grid = [16, 32, 64]

results, best = grid_search(X_raw, y_raw, lr_grid, batch_grid)

print(f"\nлучшие параметры: lr={best['lr']}  batch={best['batch']}")
print(f"средняя точность: {best['mean_acc']:.4f} ± {best['std_acc']:.4f}")

# финальная модель на всех данных
final_model, final_mean, final_std = train_final_model(
    X_raw, y_raw, lr=best['lr'], batch_size=best['batch']
)
print(f"финальная модель accuracy: {final_model.accuracy((X_raw - final_mean) / final_std, y_raw):.4f}")